...
import autoservis.servis.dao.PredracunDao;
...
public class FinansijskiPdfPreviewController {

    private static final Logger logger = Logger.getLogger(FinansijskiPdfPreviewController.class.getName());
    ...
    public FinansijskiPdfPreviewController(Predracun predracun, Klijent klijent, Vozilo vozilo,
    ...
    public void show() {
        ...
        try {
            privremeniPdf = System.getProperty("java.io.tmpdir") +
                    "/preview_pr_" + predracun.getBrojPredracuna().replace("-", "_") + ".pdf";
            FinansijskiPdfGenerator.generisiPredracunNaPutanju(predracun, klijent, vozilo, firma, privremeniPdf);
            ucitajStranice();
        } catch (Exception e) {
        ...
        Label lblNaslov = new Label("Predračun — " + predracun.getBrojPredracuna());
    ...
    private void sacuvajPdf(Stage stage) {
        ...
        fc.setTitle("Sačuvaj predračun");
        fc.setInitialFileName("PR_" + predracun.getBrojPredracuna().replace("-", "_") + ".pdf");
    ...
    private void posaljiEmail(Stage stage) {
        ...
        try {
            String naslov = firma.getEmailNaslovPredracun();
            if (naslov == null || naslov.isBlank())
                naslov = "Predračun " + predracun.getBrojPredracuna();

            String telo = firma.getEmailTekstPredracun();
            if (telo == null || telo.isBlank())
                telo = "U prilogu se nalazi predračun " + predracun.getBrojPredracuna() + ".";
            ...
            new PredracunDao().promeniStatus(predracun.getId(), "Poslat");
