...
public class NivelacijaPdfPreviewController {

    private final Nivelacija nivelacija;
    ...
    public NivelacijaPdfPreviewController(Nivelacija nivelacija) {
    ...
    public void show() {
        ...
        stage.setTitle("Nivelacija cena — " + nivelacija.getBroj());

        List<NivelacijaStavka> stavke;
        Podesavanja firma;
        try {
            stavke = new NivelacijaDao().vratiStavke(nivelacija.getId());
        ...
        try {
            privremeniPdf = System.getProperty("java.io.tmpdir") + "/preview_nivelacija_" + nivelacija.getId() + ".pdf";
            NivelacijaPdfGenerator.generisiNaPutanju(nivelacija, stavke, firma, privremeniPdf);
            ucitajStranice();
        } catch (Exception e) {
        ...
        Label lblNaslov = new Label("Nivelacija " + nivelacija.getBroj() + "  ·  " + nivelacija.getDatum());
    ...
    private void sacuvajPdf(Stage stage) {
        ...
        fc.setInitialFileName("Nivelacija_" + nivelacija.getBroj().replace("/", "-") + ".pdf");
