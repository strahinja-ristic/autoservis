...
public class PopisPdfPreviewController {
    ...
    public PopisPdfPreviewController(List<Artikal> artikli, Podesavanja firma) {
    ...
    public void show() {
        ...
        stage.setTitle("Popis skladišta");

        try {
            privremeniPdf = System.getProperty("java.io.tmpdir") + "/preview_popis.pdf";
            PopisPdfGenerator.generisiNaPutanju(artikli, firma, privremeniPdf);
            ucitajStranice();
        } catch (Exception e) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Greška");
            alert.setContentText("Greška pri generisanju popisa: " + e.getMessage());
            alert.showAndWait();
            return;
        }
        ...
        Label lblNaslov = new Label("Popis skladišta — " +
    ...
    private void sacuvajPdf(Stage stage) {
        ...
        fc.setTitle("Sačuvaj popis");
        fc.setInitialFileName("Popis_" + LocalDate.now().format(DateTimeFormatter.ofPattern("dd_MM_yyyy")) + ".pdf");
        ...
        if (odabraniFajl != null) {
            try {
                ...
                info.setContentText("Popis je sačuvan:\n" + odabraniFajl.getAbsolutePath());
                info.showAndWait();
                stage.close();
            } catch (Exception e) {
